-- Opcode alinhado ao servidor Canary (Config.TasksOpCode = 130)
local OPCODE = 130
...
function init()
  ...
  -- Otclient Redemption: registerExtendedJSONOpcode trata S/P/E e decode automaticamente
  ProtocolGame.registerExtendedJSONOpcode(OPCODE, onExtendedJSONOpcode)
...
function create()
  ...
  tasksButton = modules.game_mainpanel.addToggleButton("tasksButton", tr("Tasks"), "/images/options/button_questlog", toggle, false, 5)
...
-- Callback do Otclient Redemption: json_data já vem decodificado (suporta mensagem única e S/P/E)
function onExtendedJSONOpcode(protocol, opcode, json_data)
  ...
  if action == "config" then
    onTasksConfig(data or {})
  elseif action == "tasks" then
    onTasksList(data or {})
  elseif action == "active" then
    onTasksActive(data or {})
  elseif action == "update" then
    onTaskUpdate(data or {})
  elseif action == "points" then
    onTasksPoints(tonumber(data) or 0)
...
function onTasksActive(data)
  if not trackerWindow or not trackerWindow.contentsPanel then return end
...
function cancel(taskId)
  ...
  local protocolGame = g_game.getProtocolGame()
  if protocolGame then
    protocolGame:sendExtendedOpcode(OPCODE, json.encode({action = "cancel", data = {taskId = taskId}}))
  end
end
